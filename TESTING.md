...
`test:type-safety` uses the TypeScript parser, not line regexes. Its direct adversarial corpus separates executable type escapes from comment prose and string/template/regex contents, while retaining compiler-recognized suppression comments and multiline/parenthesized assertion coverage. The package URL guard likewise tracks proven file-URL origins and aliases, so Windows-unsafe `.pathname` conversion is rejected without banning legitimate HTTP(S) pathname validation.

The local-tarball consumer test does not borrow a warm user npm cache. It first proves a truly empty isolated cache fails with `ENOTCACHED`, then packages the installed `turndown` production closure into a temporary loopback-only registry and seeds a separate empty task-owned cache. After the registry is closed, consumer installation runs with `--offline`; the test checks exact Turndown/Domino versions and performs a real HTML-to-Markdown conversion to prove the transitive dependency loads. Missing registry inputs, failed cache preparation, or missing runtime dependencies are hard failures.
