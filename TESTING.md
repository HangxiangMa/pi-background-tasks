...
The package URL guard uses a finite flow-sensitive abstract state at each use, not whole-file assignment history. Sequential overwrites replace prior provenance, feasible branches merge, and any explicitly traced may-file branch is rejected. URL objects and statically absolute first arguments—including `import.meta.url` and static-scheme templates—override an irrelevant base exactly as WHATWG URL does. Direct/computed `pathname` access and declaration/assignment destructuring are covered while known HTTP(S) values and shadowed constructors remain controls. Calls, interprocedural/later-closure effects, heap/proxy mutation, and arbitrary dynamic strings are not executed or inferred; this is a bounded source-policy guard, not complete JavaScript dataflow analysis.

The local-tarball consumer test does not borrow a warm user npm cache or npm configuration. It first proves a truly empty isolated cache fails with `ENOTCACHED`, then packages the installed `turndown` production closure into a temporary loopback-only registry and seeds a separate empty task-owned cache. Every npm subprocess uses explicit task-owned user/global config, a task-owned project cwd with empty project config, `GIT_ALLOW_PROTOCOL=file`, and absolute filesystem operands. A no-request config probe demonstrates that a hostile scoped global/project registry would win without isolation but is absent from the effective preparation environment. After the registry is closed, consumer installation runs with `--offline`; the test checks exact Turndown/Domino versions and performs a real HTML-to-Markdown conversion to prove the transitive dependency loads. Missing registry inputs, failed cache preparation, or missing runtime dependencies are hard failures.
