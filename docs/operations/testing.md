...
- `test:package` uses a finite, flow-sensitive TypeScript analysis before rejecting `.pathname`. It respects writes before each use, merges feasible branch values, applies URL absolute-first semantics to URL objects, `import.meta.url`, literals, and static-scheme templates, and recognizes direct/computed `pathname` access plus declaration/assignment destructuring. Any statically traced may-file branch is rejected; known HTTP(S) values and shadowed URL bindings remain allowed. The guard does not execute calls or model interprocedural/later-closure effects, heap mutation, proxies, or arbitrary dynamic strings, so it is a conservative source-policy check rather than complete JavaScript dataflow proof.
