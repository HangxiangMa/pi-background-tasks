...
On session shutdown, one early synchronous lifecycle fence permanently closes the activation's task admissions, aborts each admission-owned cancellation scope, closes registry publication and the EventBus service, clears pending publication retry/status timers, suppresses completion notifications, closes every enabled Fusion/delegate/result lazy lane, aborts Fusion controllers, and aborts delegate preparation. The entrypoint joins this fence before any facade registers asynchronous cleanup, and later registrars add callbacks to the same fence rather than relying on handler order. Therefore Fusion settlement cannot pause sequential Pi dispatch while another old facade remains open. Later cleanup handlers wait for workflow/preparation/admission-owned subprocess/file/managed cleanup, kill every registry-owned running task with reason `Killed during Pi session shutdown/reload`, and report cleanup failures through the UI when possible. Preflight that crosses closure cannot insert/spawn or return EventBus success.
