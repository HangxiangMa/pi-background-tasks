...
On session shutdown, an early synchronous lifecycle barrier permanently closes admissions and aborts each admission-owned cancellation scope. Only `reason:"reload"` may first detach/remove admission-committed `surviveReload:true`, `isAgent:false` ordinary executions. Detach removes the old host adapter before any await and preserves their logical publication/notification state. The barrier then closes old registry publication and EventBus, clears pending retry/status handles, and suppresses old-host notifications. It is registered before managed-workflow shutdown handlers, so Fusion/delegate/attested/default/agent work cannot escape cleanup. The later handler drains admission-owned subprocess/file/managed cleanup, kills every remaining registry-owned running task with the specific lifecycle reason, and reports cleanup failures through the UI when possible.
