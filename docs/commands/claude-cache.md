...
Show or change the Anthropic cache-retention preference for the current session. The command is registered only when `PI_BG_FEATURES` includes the ambient `attribution` capability (enabled by default); disabling that capability removes the command together with the parent provider/hooks.
